    ...
    return (
        <div className="scrumb py-2 bg-slate-100">
            <p className="px-5 font-medium">
                <Link className="hover:text-blue-300" href="/admin">Dashboard</Link> / <span className="font-bold">Brand</span>
            </p>
        </div>
    )
}

function BrandItem({ brands }) {
    if (brands.length > 0)
        return (
            <>
                {brands.map((brand, index) => {
                    return (
                        <tr className="admin-brand__item border-b">
                            <td className="font-bold">{index + 1}</td>
                            <td className="admin-brand__item__image py-2">
                                <img
                            ...
                            <td className="admin-brand__item__info">
                                <h1 className="admin-brand__item__info__name">{brand.name}</h1>
                            </td>
                            <td className="admin-brand__item__description">
                                <h1 className="admin-brand__item__info__name">{brand.description}</h1>
                            </td>
                            <td className="admin-brand__item__action flex justify-center">
                                <Link className="mt-4 hover:bg-green-500 rounded-md" href={"/admin/brand/edit/?id=" + brand.id}>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        fetch(`${Api()}/brand/delete/${brand.id}`, {
                                            method: 'DELETE',
                                        })
...
export default function Brand() {
    ...
    useEffect(() => {
        if (search != '') {
            fetch(`${Api()}/brands/?pageSize=50&search=${search}`)
                .then((res) => res.json())
    ...
    return (
        <div className="admin-brand">
            {Scrumb()}
            <div className="new-brand__container my-2 flex justify-between">
                <Link className="text-center font-bold hover:text-blue-300" href="/admin/brand/create">Thêm sản phẩm mới(+)</Link>
            ...
            <div className="admin-brand__container border">
                <table className="text-center">
                    <thead>
                        <tr className="border-b">
                            <th className="px-16 py-5">#</th>
                            <th className="px-16 py-5">Hình ảnh</th>
                            <th className="px-16 py-5">Tên nhãn hàng</th>
