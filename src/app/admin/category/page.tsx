    ...
    return (
        <div className="scrumb py-2 bg-slate-100">
            <p className="px-5 font-medium">
                <Link className="hover:text-blue-300" href="/admin">Dashboard</Link> / <span className="font-bold">Category</span>
            </p>
        </div>
    )
}

function ProductCategoryItem({ categories }) {
    if (categories.length > 0)
        return (
            <>
                {categories.map((category, index) => {
                    return (
                        <tr className="admin-category__item border-b">
                            <td className="font-bold">{index + 1}</td>
                            <td className="admin-category__item__info">
                                <h1 className="admin-category__item__info__name">{category.name}</h1>
                            </td>
                            <td className="admin-category__item__description">
                                <h1 className="admin-category__item__description-content">{category.description}</h1>
                            </td>
                            <td className="admin-category__item__quantity">{category.quantity}</td>

                            <td className="admin-category__item__action flex justify-center">
                                <Link className="mt-4 hover:bg-green-500 rounded-md" href={"/admin/category/edit/?id=" + category.id}>
...
export default function Product() {
    ...
    useEffect(() => {
        if (search != '') {
            fetch(`${Api()}/categories/?pageSize=50&search=${search}`)
                .then((res) => res.json())
    ...
    return (
        <div className="admin-product">
            ...
            <div className="new-product__container my-2 flex justify-between">
                <Link className="text-center font-bold hover:text-blue-300" href="/admin/category/create">Thêm danh mục mới(+)</Link>
                ...
                <table className="text-center">
                    <thead>
                        <tr className="border-b">
                            ...
                            <th className="px-16 py-5">Tên danh mục</th>
                            <th className="px-16 py-5">Mô tả</th>
                            <th className="px-16 py-5">Số lượng hàng</th>
                            <th className="px-16 py-5">Thao tác</th>
                        </tr>
                    </thead>
                    <tbody>
                        {ProductCategoryItem({ categories })}
                    </tbody>
                </table>
